...
import { updateActivityFactor, updateInjuryFactor, updateMolecularCalc, updateMolecularDistribution } from '../../../redux/slices/diet';
import { calculateMolecularCalc, calculateMolecularDistribution } from '../../../utils/handlers';
import { ACTIVITY_FACTOR, formatNumber, INJURY_FACTOR, INJURY_VALUE } from '../../../utils';
import { Select } from '../../../components/Select';

export const UpdateMolecular = ({
  open,
  handleClose,
  onAccept
}) => {
  ...
  const onUpdateMolecularCalc = ({target: {value, name}}) => {
    dispatch(updateMolecularCalc(calculateMolecularCalc({
      ...molecularCalc,
      [name]: value
  ...
  const onUpdateDistribution = (newData) => {
    dispatch(
      updateMolecularDistribution(
        calculateMolecularDistribution(newData, RCT)
      )
    );
  }
  ...
  useEffect(() => {
    if(activityFactor && injuryFactor){
      dispatch(updateMolecularCalc(calculateMolecularCalc(molecularCalc, activityFactor, injuryFactor)));
    }
  }, [activityFactor, injuryFactor])
        ...
        <Box mt={2} mb={2} display={'flex'}>
          <Box width={"50%"} mr={1}>
            <Select
              ...
              onChange={(value) => {
                console.log(`activity_factor_value:`, value)
          ...
          <Box width={"50%"} ml={1}>
            <Select
              ...
              onChange={(value) => {
                console.log(`injury_factor_label: `, value);
                dispatch(updateInjuryFactor({
